/**
 * quoteDialog.js is the handler for the 'Change Schedule' dialog for Daily Quote
...
module.exports = async (event, context) => {
  ...
  let time = `${event.submission.dailyQuoteTime}`,
      days = `${event.submission.dailyQuoteDays}`;

  // Update the database
  let updatingUserRecord = await query.update({
    table: `Wellness Subscribers`,
    where: {
      'User ID': `${event.user.id}`
    },
    fields: {
      'Daily Quote Time': parseInt(time),
      'Daily Quote Days': parseInt(days)
    }
  });
  ...
  let response = await axios.request({
    ...
    data: JSON.stringify({
      ...
      attachments: [
        helper.dailyQuoteAttachment(user.dailyQuote, user.dailyQuoteDays, dailyQuoteTime), //attachment that is changing
