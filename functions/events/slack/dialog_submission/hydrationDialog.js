/**
 * hydrationDialog.js is the handler for the 'Change Schedule' dialog for Hydration
...
module.exports = async (event, context) => {
  ...
  let days = `${event.submission.hydrationDays}`,
      startTime = `${event.submission.hydrationStartTime}`,
      endTime = `${event.submission.hydrationEndTime}`,
      interval = `${event.submission.hydrationInterval}`;
  ...
  if (errorsFound) {
    ...
    let times = helper.calculateTimes(startTime, endTime, interval);
    
    // just in case catchErrors didn't find this error. We never want the times array to be stored as null
    if (times != null) {
      // Update the database
      await query.update({
        ...
        fields: {
          'Hydration Times': times,
          'Hydration Interval': parseInt(interval),
          'Hydration Days': parseInt(days),
          'Hydration End': parseInt(endTime)
        }
      });
    }
    else {
